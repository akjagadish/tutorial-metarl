...
class EarlyStopping:
    ...
    def __init__(self, mode='min', model_path=None, patience=7, verbose=False, delta=0., update_best_score=True):
        """
        Args:
    ...
    def check_min(self, current_score, model):
        ...
        if (self.best_score + self.delta) < current_score:
            ...
            self.best_score = current_score if self.update_best_score else self.best_score
    ...
    def __call__(self, current_score, model):
        ...
        if self.best_score is None:
            ...
            self.check_min(current_score, model) if self.mode == 'min' else self.check_max(current_score, model)

    def save_checkpoint(self, new_score, model):
        '''Saves model when validation loss decrease.'''
        if self.verbose:
            print(f'Score improved ({self.last_best_score:.6f} --> {new_score:.6f}).', flush=True)
