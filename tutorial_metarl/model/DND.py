...
class DND():
    ...
    def __init__(self, dict_len, memory_dim, kernel='l2', unique_keys=False):
        # params
    ...
    def save_memory(self, memory_key, memory_val):
        ...
        if self.unique_keys and len(self.keys) >= 1:
            # if memory_key in self.keys:
            #    self.vals[self.keys.index(memory_key)] = memory_val
            unique = True
            for idx, key in enumerate(self.keys):
                
                if torch.equal(memory_key, key.reshape(-1)):
                    self.vals[idx] = torch.squeeze(memory_val.data)
                    unique = False
            if unique: # and not torch.equal(memory_key, torch.as_tensor([1.,1.])):
    ...
    def get_memory(self, query_key, policy='1NN'):
        ...
        best_memory_val = self._get_memory(similarities, policy)
    ...
    def _get_memory(self, similarities, policy='1NN'):
        ...
        if policy == '1NN':
            ...
            if torch.sum(similarities)>0.0:
                weights = similarities/torch.sum(similarities)
            best_memory_val = compute_weighted_sum(weights, self.vals) 
        elif policy == 'softmax':
            weights = softmax(similarities, beta=1)
            best_memory_val = compute_weighted_sum(weights.reshape(-1), self.vals)
        else:
...
def compute_weighted_sum(weights, values):
    """[summary]
    ...
    """
    retreive_memory_val = None
    for idx in range(np.shape(weights)[0]):
        if retreive_memory_val is None:
            retreive_memory_val = weights[idx]*values[idx]
        else:
            retreive_memory_val += weights[idx]*values[idx]
    return retreive_memory_val